...
def add_item(order_id, item_id):
    return cmi.get_status("INSERT INTO order_items (order_id, item_id, count) VALUES (%s,%s,1) ON CONFLICT (order_id, item_id) DO UPDATE SET count = order_items.count+1", [order_id, item_id], g.cm)
...
def remove_item(order_id, item_id):
    ...
    if status_code != 200:
        ...
        return '{"done": false}', 400
    if data["count"] == 0:
        cmi.exec("DELETE FROM order_items WHERE order_id=%s AND item_id=%s", [order_id, item_id], g.cm)
...
def find_order(order_id):
    return cmi.get_one("SELECT %s AS order_id, (SELECT paid FROM order_headers WHERE order_id=%s) AS paid, coalesce(json_object_agg(item_id::string, count), '{}'::json) AS items, (SELECT user_id FROM order_headers WHERE order_id=%s) AS user_id FROM order_items WHERE order_id=%s", [order_id, order_id, order_id, order_id], g.cm)
...
def checkout(order_id):
    ...
    data, status_code = cmi.get_one("SELECT user_id FROM order_headers WHERE order_id=%s", [order_id], g.cm)
    if status_code != 200:
        if not g.already_using_connection_manager:
            cmi.cancel_tx(g.cm)
        return '{"done": false}', 400
    ...
    for item_id, count in items.items():
        response = requests.get(f"{stock_url}/find/{item_id}", headers={"cm": ':'.join(cm)})
        if response.status_code != 200:
            if not g.already_using_connection_manager:
                cmi.cancel_tx(g.cm)
            return '{"done": false}', 400
        ...
        response = requests.post(f"{stock_url}/subtract/{item_id}/{count}", headers={"cm": ':'.join(cm)})
        if response.status_code != 200:
            if not g.already_using_connection_manager:
                cmi.cancel_tx(g.cm)
            return '{"done": false}', 400

    response = requests.post(f"{payment_url}/pay/{user_id}/{order_id}/{amount}", headers={"cm": ':'.join(cm)})
    if response.status_code != 200:
        if not g.already_using_connection_manager:
            cmi.cancel_tx(g.cm)
        return '{"done": false}', 400
